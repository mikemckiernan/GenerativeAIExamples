...
class Message(BaseModel):
    ...
    role: str = Field(description="Role for a message AI, User and System", default="user")
    content: str = Field(description="The input query/prompt to the pipeline.", default="I am going to Paris, what should I see?")
...
class ChainResponse(BaseModel):
    ...
    choices: List[ChainResponseChoices] = Field(default=[])

class DocumentSearch(BaseModel):
    """Definition of the DocumentSearch API data type."""

    query: str = Field(description="The content or keywords to search for within documents.")
    top_k: int = Field(description="The maximum number of documents to return in the response.", default=4)
...
class DocumentsResponse(BaseModel):
    ...
    documents: List[str] = Field(..., description="List of filenames.")
...
def import_example() -> None:
    ...
    app.openapi_schema = {
        **app.openapi(),
        "servers": [{"url": "http://localhost:8081", "description": "Chain server"}],
    }
...
async def get_documents(request: Request) -> DocumentsResponse:
    ...
    try:
        ...
        logger.error(f"Error from /list endpoint. Error details: {e}")
